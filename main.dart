  ...
  List<Widget> listaBotonesInvernaderos() {
    ...
    for (int i = 0; i < managerInvernaderos.getNumInvernaderos(); i++) {
      botones.add(ElevatedButton(
              onPressed: () {
                _seleccionarInvernadero(i);
              },
              child: const Text("Botón que representa un invernadero"),
            ));
    }
    return botones;
  }

  Text textoInfoInvernadero() {
    String? texto = invernaderoSeleccionado?.infoInvernadero();
    if (texto != null) {
      return Text(texto);
    }
    return Text("Seleccionar un invernadero para ver sus datos.");
  }
